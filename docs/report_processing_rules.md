...
- **Match Termination:**
  ...
  - If a rule yields exactly `1` match, execution immediately HALTS and returns that match. The output will include `"match_rule": "<Step ID>"` (e.g., `"match_rule": "A1"`).
  - If a rule yields `0` matches, or `> 1` matches, execution proceeds to the next rule in the sequence, logging a `No_Match_Reason` explanation internally.
  - If all rules are exhausted without finding exactly 1 match, the system returns `null` (No Match) and provides the `No_Match_Reason` in the response metadata.
- **Data Sanitization:**
...
When a match is found, the system extracts the following columns from the Oracle report and appends them to the API response:
* `fusion_receipt_number` <-- `RECEIPT_NUMBER`
* `fusion_receipt_date` <-- `RECEIPT_DATE`
* `fusion_customer_name` <-- `BILL_CUSTOMER_NAME`
* `fusion_customer_number` <-- `BILL_CUSTOMER_NUMBER`
* `fusion_currency` <-- `CURRENCY`
* `fusion_receipt_status_code` <-- `RECEIPT_STATUS_CODE`
