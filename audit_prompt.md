...
> 5. **Structural Hygiene & CI/CD:** Flag duplicated configurations, hardcoded environment fallbacks that might leak into production, misconfigured deployment scripts (e.g., `render.yaml` or Vercel `vercel.json`), missing essential API keys, and bloated dependencies.
> 6. **Test Quality & Mocking:** Identify tests that provide false positives. Specifically, look out for incorrect assertions, testing the wrong endpoint, or poorly structured `respx` or `httpx` mocks (e.g. using overly broad regexes that mask missing protocol schemes, or mocks that return JSON when XML/CSV is expected). Ensure that test environment variables (like URLs) mimic production (e.g. contain valid schemes like `https://` instead of `"None"` to avoid protocol errors that falsely pass due to caught exceptions).
> 
