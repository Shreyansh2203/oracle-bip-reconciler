...
async def mine_customer_data(
    client: httpx.AsyncClient, username: str, password: str, customer_name: str, sem: asyncio.Semaphore
) -> None:
    async with sem:
        try:
...
async def main():
    ...
    with open(CUSTOMERS_FILE, encoding="utf-8") as f:
        lines = f.readlines()[1:]
        all_customers = list({line.strip() for line in lines if line.strip()})
