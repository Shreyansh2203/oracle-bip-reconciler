...
def test_string_sanitization():
    # "none" strings for text fields should become empty strings
    req = ReconciliationRequest(
        customer_name="none",
        payment_reference=" NONE ",
        payment_date="None"
    )
    assert req.customer_name == ""
    assert req.payment_reference == ""
    assert req.payment_date == ""
    ...
    assert item.invoice_number == ""
    assert item.invoice_date == ""
    assert item.customer_invoice_number == ""
